...
def test_employee_full_name():
    ...
    assert emp.full_name() == 'Ivanov Ivan Ivanovich', 'Error1'


def test_employee_birthday():
    emp = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, 'manager', 50000)
    emp.birthday()
    assert emp.get_age() == 31, 'Error2'


def test_employee_raise_salary():
    emp = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, 'manager', 50000)
    emp.raise_salary(10)
    assert emp.salary == 55000.0, 'Error3'


def test_employee_str():
    emp = Employee('Ivanov', 'Ivan', 'Ivanovich', 30, 'manager', 50000)
    assert emp.__str__()=='Ivanov Ivan Ivanovich (Manager)', 'Error4'


def test_employee_last_name_title():
    emp = Employee('ivanov', 'ivan', 'ivanovich', 30, 'manager', 50000)
    assert emp.last_name == 'Ivanov', 'Error5'
