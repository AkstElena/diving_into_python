...
control = main(factorial)  # создаем функцию замыкания, передавая в переменную control, по сути задекорированный factorial
print(f'{control.__name__ = }')  # видим что вызывается функция обертка (wrapper)
print(f'{control(100) = }')
